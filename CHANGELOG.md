...
- **OpenClaw workspace:** **`AGENTS.md`**, **`TOOLS.md`**, **`workspace/agents/comfyui-assets.md`** (and **`.example`** templates) distinguish **checkpoint pulls** (`models/`) from **proxy-only** Juno graphs — avoids bogus “model download” heartbeats for HTTP-only paths.
