...
### Added
- **SSO front door for the dashboard.** Caddy + oauth2-proxy + Google OIDC, allowlisted by `auth/oauth2-proxy/emails.txt`, bound to `${CADDY_BIND}` (your tailnet IP). The dashboard's `_verify_auth()` now accepts `X-Forwarded-Email` from `DASHBOARD_TRUSTED_PROXY_NET` and falls back to bearer for non-proxy callers. Caddy uses the `route { forward_auth + handle_path }` pattern (terminal `handle @auth` was a dead-end). Other UIs (Open WebUI, n8n, ComfyUI, Hermes) stay on direct host ports — they're SPAs that emit absolute `/assets/` URLs and can't be sub-path-mounted under a single host. Operator runbook: `docs/runbooks/auth.md`.
- **Encrypted secrets at rest with SOPS + age.** All managed tokens live in `secrets/*.sops` (committable) and decrypt to `~/.ai-toolkit/runtime/` only when `make decrypt-secrets` runs. The runtime dir is outside any container bind-mount so a prompt-injected agent can't `cat` the plaintext. High-value tokens (Discord, GitHub PAT, HF, Tavily, Civitai) are mounted as Docker secrets (file-form at `/run/secrets/<name>`), not env vars — so they don't appear in `docker inspect`. Internal tokens (`LITELLM_MASTER_KEY`, `DASHBOARD_AUTH_TOKEN`, `OPS_CONTROLLER_TOKEN`, `OAUTH2_PROXY_COOKIE_SECRET`) live in `secrets/.env.sops`; rotate all at once with `make rotate-internal-tokens`. Audit script `scripts/secrets/audit-git-history.sh` greps the full git log for known token-format prefixes. Operator runbook: `docs/runbooks/secrets.md`.
- **Audited compose lifecycle via ops-controller.** New endpoints `GET /containers`, `GET /containers/{name}/logs`, `POST /containers/{name}/restart`, `POST /compose/{up,down,restart}` (whole-stack ops require explicit `confirm=True`). Every privileged call appends one JSONL line to `data/ops-controller/audit.jsonl`, rotated to `audit.1.jsonl` at `AUDIT_LOG_MAX_BYTES` (default 50 MB). `hermes/ops_client.py` is the audited HTTP wrapper for callers that want the recorded path. Operator runbook: `docs/runbooks/bounded-hermes.md`.

### Changed
- **Default chat model swapped to Qwen3.6-35B-A3B Q4_K_M (MoE).** 35B total params, ~3B active per token across 256 experts, no SWA penalty — measurably faster generation than the prior Gemma 4 31B at similar quality. Set via `LLAMACPP_MODEL`. TurboQuant `tbqp3_0` KV cache stays the recommended pairing for Blackwell sm_120.
- **Hermes mount path is now env-var-driven.** `HERMES_HOST_DEV_MOUNT` controls where the host's project root is mirrored inside the container (default `/c/dev` on Windows, `/workspace` elsewhere) so sibling `docker compose` calls via the mounted socket find the same paths the host does. Replaces the previously-hardcoded `/projects` mount.
- Hermes Agent migrated from host-mode install to Docker compose services (`hermes-gateway` + `hermes-dashboard`). One `docker compose up -d` now brings the whole stack online atomically. Auto-restart, `depends_on: service_healthy` coordination, internal-DNS health probe from the Ordo dashboard (`http://hermes-dashboard:9119/`). Deletes `scripts/start-hermes-host.sh` and the global `hermes` wrapper at `~/.local/bin/`. Hermes runtime state lives in the named Docker volume `ordo-ai-stack_hermes-data` (mounted at `/home/hermes/.hermes`); the host `data/hermes/` directory is stale leftover from before the volume migration and is no longer mounted live.

### Removed
- **Bounded-Hermes socket removal (rolled back).** The plan to drop `/var/run/docker.sock` from `hermes-gateway` and force every privileged call through `OpsClient` was prototyped (PR #6) and merged briefly, then reverted because the upstream Hermes image's built-in `docker` / `docker compose` tools call the socket directly and have no `OpsClient` bridge yet. The socket is back; the audited HTTP API and audit log remain as an opt-in path. Path forward: a Hermes plugin that intercepts the built-in tools (`docs/runbooks/bounded-hermes.md` for design and rationale).
