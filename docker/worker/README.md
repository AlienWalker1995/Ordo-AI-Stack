...
it from the repo root with `services/worker/Dockerfile` (it bundles the dashboard package + worker deps) —
...
```
docker build -f C:/dev/ordo-ai-stack/services/worker/Dockerfile -t ordo/worker:latest C:/dev/ordo-ai-stack
```
