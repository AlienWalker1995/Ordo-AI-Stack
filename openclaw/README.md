...
- **Long-context tuning:** `openclaw-config-sync` also applies `OPENCLAW_COMPACTION_MODE`, `OPENCLAW_AGENT_TIMEOUT_SECONDS`, and `OPENCLAW_LLM_IDLE_TIMEOUT_SECONDS` from the root `.env`. For large local models, `safeguard` compaction plus a higher idle timeout prevents long prompt-prefill runs from timing out before the first token. Additionally, model-gateway enforces `OPENCLAW_MAX_INPUT_TOKENS` (default 30720) to trim oversized prompts before they reach llama.cpp, preventing slow decode regardless of the model's full context window.
