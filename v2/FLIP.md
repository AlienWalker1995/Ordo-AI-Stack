...
- `~15:13` — edge live: caddy bound tailnet `100.x.y.z:443` (both PortBindings AND NetworkSettings
  populated — no silent-loss), `https://host.tailnet.ts.net/` → **302 /oauth2/start → 302
...
- **Edge:** caddy `:443` bound to tailnet `100.x.y.z` (PortBindings AND NetworkSettings — no
  silent-loss); `https://host.tailnet.ts.net/` → **302 /oauth2/start** (Google SSO enforced),
