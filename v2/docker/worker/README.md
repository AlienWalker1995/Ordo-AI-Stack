...
V2's background job worker, referenced by the `worker` plugin as `ordo-v2/worker:latest`. V1 builds
...
```
docker build -f C:/dev/ordo-ai-stack/worker/Dockerfile -t ordo-v2/worker:latest C:/dev/ordo-ai-stack
```
