...
The plugin initialises itself headlessly — no click-through setup wizard. On the first boot against
...
| Key | Rotatable? |
|---|---|
| `LANGFUSE_DB_PASSWORD`, `LANGFUSE_CLICKHOUSE_PASSWORD`, `LANGFUSE_REDIS_AUTH`, `LANGFUSE_MINIO_SECRET`, `LANGFUSE_NEXTAUTH_SECRET` | Yes — `scripts/secrets/rotate-internal.sh` (the two databases also need an `ALTER USER` first; the script prints the steps). |
