...
- the key is generated by `ordo init` (`secrets.token_urlsafe(32)`) and rotated by
  `ordo secrets rotate --internal` (the `ordo recreate --reading` it prints covers the agent);
...
- **Preflight and mid-run guard.** Before any suite runs (and before `run_dir` even exists, the same
  ...
  `ordo/control/api.py`'s `ControlPlane`, which does not validate a token at all (other callers send
