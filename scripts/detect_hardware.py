...
def format_gpu_assignments(assignments: dict[str, str]) -> str:
    """Render overrides/gpu-assignments.yml pinning each service to a GPU UUID.

    Delegates to the canonical implementation in ops-controller/gpu_assignments_fmt.py
    so detect_hardware, model_registry, and main.py all emit identical YAML.
    Canonical impl: ops-controller/gpu_assignments_fmt.py

    Two layers, because container-level GPU isolation is NOT enforced under
    Docker Desktop / WSL2 (verified on ULTRACAM 2026-06-07): even with the
    device_ids reservation below, every container still sees ALL GPUs via
    nvidia-smi, and the CUDA-derived images bake NVIDIA_VISIBLE_DEVICES=all.
    The reservation pins correctly on native Linux; CUDA_VISIBLE_DEVICES is
    what actually restricts the process to the assigned GPU on WSL2 (the CUDA
    runtime honors it in-process). We emit both so the pin holds everywhere:
      - environment CUDA_VISIBLE_DEVICES / NVIDIA_VISIBLE_DEVICES = GPU-<uuid>
      - deploy.resources.reservations.devices[].device_ids = [GPU-<uuid>]
    """
    return _render_gpu_yaml(assignments)
...
def build_overrides(
    *,
    ...
    comfyui_mem: str,
    embed_mem: str,
    common_sidecars: dict,
) -> dict:
    """Return the full overrides dict for all GPU modes.

    For nvidia mode: GPU compute reservations (driver: nvidia, count: all,
    capabilities: [gpu]) are intentionally ABSENT from llamacpp, llamacpp-embed,
    and comfyui. Those services are pinned by uuid in overrides/gpu-assignments.yml
    (Task 2) to avoid Docker Compose sequence-concatenation conflicts. Only the
    dashboard service retains a utility-only reservation for NVML stats.
    """
    return {
        "nvidia": {
            "llamacpp": {
...
def main() -> int:
    ...
    overrides = build_overrides(
        ...
        embed_mem=embed_mem,
        common_sidecars=common_sidecars,
    )
