...
def test_job_bigger_than_gpu_is_removed_not_left_to_starve_the_queue():
    # A job that can never fit is rejected rather than blocking the queue head.
...
def test_broker_does_not_start_a_rejected_job():
    ...
    assert b.backend.started == ["chat"]
    assert s.status()["rejected"] == ["huge"]


def test_there_is_no_cloud_fallback():
    """cloud_fallback routed a too-big job to a drain route nothing polled, and the lease clients
    only watch `rejected`, so they waited out their timeout. A too-big job is always rejected."""
    assert "cloud_routed" not in Scheduler(32).status()
    with pytest.raises(TypeError):
        Scheduler(32, cloud_fallback=True)
