"""The control-plane UI is pluggable data (like agents): native is the default; a deployment
...
def test_native_is_the_default():
    ...
    assert d is not None and d.id == "native" and d.default


def test_native_image_convention():
    d = DASHBOARDS.get("native")
...
def test_unknown_dashboard_falls_back_to_default_with_warning():
    ...
    assert d is not None and d.id == "native"  # falls back to default (dashboard is not optional)
    assert any("not in the registry" in n for n in notes)


# ── default render (native) stays exactly as before ──────────────────────────
def test_default_render_uses_native_spa(tmp_path):
    c = _compose("native", tmp_path)
...
def test_native_dashboard_still_gates_on_scheduler(tmp_path):
    c = _compose("native", tmp_path)
...
def test_scheduler_service_is_untouched_by_dashboard_choice(tmp_path):
...
def test_native_dashboard_reserves_no_gpu(tmp_path):
