...
SERVER_IDS = {"codebase-memory", "comfyui", "memory-vault", "n8n", "orchestration", "qdrant-rag", "searxng"}
...
def test_strip_prefix_removes_only_a_known_server_prefix():
    assert strip_prefix("memory-vault-read_note", SERVER_IDS) == "read_note"
    assert strip_prefix("qdrant-rag-qdrant_search", SERVER_IDS) == "qdrant_search"
    # an unknown prefix is left alone (never guess)
    assert strip_prefix("other-tool", SERVER_IDS) == "other-tool"
    # the longest matching server id wins (memory-vault, not a hypothetical `memory`)
...
def test_compare_is_a_superset_check_with_required_extras():
    ...
    current = [f"memory-vault-{n}" if n == "read_note" else f"searxng-{n}" for n in baseline]
