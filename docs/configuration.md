...
> Configuration is **rendered, not hand-edited**: the declarative source `ordo.yaml` (template `ordo.example.yaml`) is regenerated by the render engine into `out/.env`, `out/docker-compose.yml`, `out/manifest.json`, etc. The `overrides:` block in the source is the escape hatch for values that must survive a re-render; hand-editing `out/*` does not. Secrets live in a gitignored `out/secrets.env` (keys listed in the rendered `out/secrets.env.example`), not in a root `.env`. GPU/compute is decided by the render engine's hardware detection (`hardware: auto` in `ordo.yaml`, or `ordo detect`). Bring-up runs from the repo root (`ordo up --all`). Authoritative guide: [`operator-guide.md`](operator-guide.md) and `ordo.example.yaml`.
...
| `DISCORD_BOT_TOKEN` | *(empty)* | Discord bot token. Managed via SOPS (`secrets/discord_token.sops`) or `DISCORD_BOT_TOKEN_FILE=/run/secrets/discord_bot_token`; inline `DISCORD_TOKEN=` in `.env` is no longer accepted. |
...
| Variable | Purpose |
|---|---|
| `LANGFUSE_DB_PASSWORD` | `langfuse-db` Postgres password (a file for `langfuse-db`; compose-interpolated into `DATABASE_URL` for langfuse-web/worker, which have no file option) |
...
| `GOOGLE_CLIENT_ID_FILE` | `/run/secrets/oauth2_proxy_client_id` | The edge's existing Google OAuth client id, as a file; the entrypoint exports it as `GOOGLE_CLIENT_ID` |
| `GOOGLE_CLIENT_SECRET_FILE` | `/run/secrets/oauth2_proxy_client_secret` | The same client's secret, as a file; the entrypoint exports it as `GOOGLE_CLIENT_SECRET` |
