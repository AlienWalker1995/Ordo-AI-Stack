...
> ⚠️ **LEGACY (V1) — superseded by v2/ on 2026-07-09.** In the production **v2** stack, configuration is **rendered, not hand-edited**: one declarative source (`v2/ordo.yaml`) is regenerated by the render engine into `v2/out/.env`, `v2/out/docker-compose.yml`, `v2/out/hermes.context.json`, etc. — so the drift this page's manual `.env` / `overrides/compute.yml` model invited is structurally impossible. Editing `v2/out/*` does not survive a re-render (the `overrides:` block in the source is the escape hatch). Secrets live in a gitignored `v2/out/secrets.env` (rendered from `secrets.env.example`), **not** in `.env`. GPU/compute is decided by the `ordo serve` scheduler, not `COMPUTE_MODE` + a reactive guardian. For current config, see **[`../v2/README.md`](../v2/README.md)** and `v2/ordo.example.yaml`. The reference below describes the retired V1 `.env`-driven model and is kept for historical reference. See [`LEGACY-CLEANUP.md`](LEGACY-CLEANUP.md).
