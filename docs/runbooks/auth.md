...
| 502 from Caddy on a UI port | Service not on `ordo-net` (the single rendered network) or its container is down | `ordo recreate <svc>` (for caddy it recreates the netns members too, never `--no-deps`) |
