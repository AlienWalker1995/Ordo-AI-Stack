...
| 401 / `AUTH_FAIL` on every `/api/*` call from `/dash/` | Caddy is on multiple shared networks with the dashboard, Docker DNS returned the non-`proxy-net` IP, dashboard's `_request_from_trusted_proxy` rejected it | Caddy should be on `proxy-net` *only*; verify with `docker inspect ordo-ai-stack-caddy-1 --format '{{json .NetworkSettings.Networks}}'`. All upstreams Caddy proxies are on `proxy-net`, so `frontend` membership is unnecessary and harmful |
