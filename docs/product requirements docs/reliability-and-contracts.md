...
## Positioning: Agents as Mesh Consumers
...
1. **Preflight** — Agents (and tooling) know whether downstream services are **healthy** before attempting dependent actions.
2. **Isolation** — One bad MCP server, one model timeout, or one broken bridge does **not** poison all tool or model usage.
3. **Traceability** — Every user-visible failure is attributable to a **layer** (agent, model gateway, provider, MCP gateway, tool server, browser bridge).
...
Requirements: explicit browser session lifecycle APIs; health check separate from any agent's UI; idle timeouts / leases; diagnostics events; recycle crashed sessions; operator metrics. Browser automation is optional, not a hard dependency for chat.

## Observability (Elite Bar)

- **Golden trace:** One `request_id` from agent ingress through model selection, model gateway, provider, MCP gateway, tool server, optional browser bridge.
...
- **SLO-style reporting:** model gateway success rate, p50/p95 latency, MCP tool success rate, top failing tools, per-agent request success rate.

## Configuration Management

- Version and validate config on startup; deprecations and safe auto-migration.
- Agent integration checks: model gateway endpoint reachable, tokens valid, default model exists, required MCP tools enumerated, browser port assumptions.
...
| Phase | Focus | Outcome |
|-------|--------|---------|
| **1** | Visibility | Typed readiness, registry, correlation, taxonomy, dashboard dependency page, agent startup validation, E2E smoke tests |
